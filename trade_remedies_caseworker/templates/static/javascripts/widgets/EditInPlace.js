...
define(function () {
  ...
  function startEdit(state) {
    ...
    if (!this.editElement) {
      ...
      this.saveCancel = $(
        '<div class="edit-buttons"><button class="save"><img class="icon icon-tick"></button><button class="cancel"><img class="icon icon-cross"></button></div>'
      );
      this.el.prepend(this.saveCancel);
      // attach handlers
      this.saveCancel.find(".save").on("click", function () {
        stopEdit.call(self, true);
      });
      this.saveCancel.find(".cancel").on("click", function () {
        stopEdit.call(self);
      });
      ...
      this.sources.each(function (idx) {
        ...
        editElement.css({
          ...
          resize: "none",
          zIndex: "1",
          overflowY: "scroll" /*,minHeight:'200px'*/,
        });
