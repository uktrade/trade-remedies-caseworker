define(function () {
  ...
  function onKeypressDef(e) {
    ...
    if (idx > 0 && val == "" && e.which === 8) {
      // backspace on empty field goes back one
      this.parts[idx - 1].select();
    }
  }
  function onKeyup(e) {
    // needed to trap the backspace
    if (e.which === 8) {
      return false;
  ...
  function onKeypress(e) {
    var self = this;
    var evt = e;
    if (evt.which > 32 && /\D/.test(String.fromCharCode(evt.which)))
      return false; // only let numeric chars through
    _.defer(function () {
