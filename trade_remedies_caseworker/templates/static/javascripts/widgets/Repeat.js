define(["modules/helpers"], function (helpers) {
  ...
  var constructor = function (el) {
    ...
    this.container[this.blockCount == 1 ? "addClass" : "removeClass"](
      "hide-remove-buttons"
    );
  ...
  function buildBlock(block) {
    var self = this;
    var container = $(
      '<div class="repeat-block"><div class="inner"><div class="question-list"></div><button class="button pull-right btn-remove" type="button">Remove</button></div></div>'
    );
    _.each(block, function (el) {
      container.find(".inner .question-list").append(el);
    });
    container.find("button.btn-remove").on("click", function () {
      removeClick.call(self, this);
    });
    self.blockCount++;
    return container;
  }
