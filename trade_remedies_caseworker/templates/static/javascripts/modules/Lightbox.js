define(["modules/overlay"], function (overlay) {
  ...
  var template = _.template(
    '<% if(title) { %><div class="title"><h1><%= title %></h1></div><% } %><div class="outer"><span><%= (message && (typeof(message) == "function" ? message(obj.data) : message.replace("script", ""))) || "" %></span></div>\
        ...
        <% if(buttons.cancel){ %><button type="button" value="" class="button button-grey compact dlg-close pull-right"><%=buttons.cancel.txt || "Cancel" %></button><% } %>\
        <% if(buttons.close){ %><button type="button" value="" class="button button-grey compact dlg-close pull-right">Close</button><% } %>\
        <% if(buttons.yes){ %><button type="button" value="yes" class="button button-blue compact dlg-close pull-right"><%=buttons.yes.txt || "Yes" %></button><% } %>\
        <% if(buttons.no){ %><button type="button" value="no" class="button btn-blue compact dlg-close pull-right"><%=buttons.no.txt || "No" %></button><% } %>\
        </div>'
  );
  ...
  var constructor = function (options) {
    ...
    if (!this.options.content) {
      // build a dialoge from the templates
      this.options = _.extend(
        { buttons: { ok: 1 }, type: { infonly: 1 } },
        options
      );
    ...
    if (options && options.type && options.type.infonly)
      this.options.type.infonly = this.options.options.type.infonly || null; // what?
    ...
    this.$el = $(
      '<div aria-live="assertive" role="alert" class="pop-up' +
        bsClass +
        '" tabindex="-1"></div>'
    );
    ...
    if (this.options.addClose) {
      ...
      this.$header.append(
        '<button class="dlg-close"><span class="icon icon-cross"></span><span class="sr-only">Close the dialogue</span></button>'
      );
    }
    ...
    // };
    this.$overlay.setClass("overlay", !this.options.nonmodal);
    this.$overlay.append(this.$el).prependTo(document.body);
    ...
    this.$el
      .on("click", function (e) {
        //e.stopPropagation();
        var closeButton = $(e.target).closest(".dlg-close");
        if (closeButton.length > 0) {
          self.close();
          if (closeButton.attr("redirect_on_close")) {
            document.location.replace(closeButton.attr("redirect_on_close"));
          }
        }
      })
      .on("keyup", function (e) {
  ...
  constructor.prototype = {
    ...
    centre: function (options) {
      ...
      if (this.$el.height() > this.$overlay.height() - gap) {
        this.$el.css({
          height: this.$overlay.height() - gap + "px",
          overflow: "auto",
        });
      }
      if (this.$el.width() > this.$overlay.width() - gap) {
        this.$el.css({
          width: this.$overlay.width() - gap + "px",
          overflow: "auto",
        });
      }
      this.$el.css({
        left: (this.$overlay.width() - this.$el.width()) / 2,
        top: (this.$overlay.height() - this.$el.height()) / 2,
      });
