define(["modules/helpers"], function (helpers) {
  ...
  function renderNotes(container) {
    ...
    if (this.questionsContainer && this.fieldNotes) {
      ...
      notesBlocks.each(function (notesBlock) {
        var block = $(this);
        block.html(
          '<div class="note-block-head"><button type="button" class="btn-add-note pull-right">Add note</button></div>'
        ); // clear the custard
  ...
  function getNotes() {
    ...
    $.ajax({
      ...
      self.noteData = result.notes.sort(function (a, b) {
      ...
      _.each(self.noteData, function (note) {
        if (note.fieldId) {
          (self.fieldNotes[note.fieldId] =
            self.fieldNotes[note.fieldId] || []).push(note);
        } else {
  ...
  function addNoteDialogue(button) {
    ...
    setTimeout(function () {
      self.addNoteFm.css({ height: "auto" });
    }, 300);
  }
