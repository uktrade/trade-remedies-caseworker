define(["modules/helpers"], function (helpers) {
  var tpText = _.template(
    '<label><%=title%></label><input type="text" name="<%=field%>" value="<%=obj[field]%>">'
  );
  var tpArea = _.template(
    '<label><%=title%></label><textarea name="<%=field%>"><%=obj[field]%></textarea>'
  );
  var tpSelect = _.template(
    '<label><%=title%></label><select name="<%=field%>" value="<%=obj[field]%>">\
  ...
  var tpCheckbox = _.template(
    '<label><%=title%></label><div><div>Yes<input class="nofloat" type="radio" name="<%=field%>" <% if(obj[field]) { print(\'checked="checked"\') } %> value="True" ></div><div>No:<input type="radio" class="nofloat" name="<%=field%>" <% if(!obj[field]) { print(\'checked="checked"\') } %> value="False" ></div></div>'
  );
  var tpRo = _.template(
    "<label><%=title%></label><span><%=obj[field]%></span>"
  );
  ...
  function editPageFull(section, details) {
    require(["modules/Lightbox"], function (Lightbox) {
      var lightbox = new Lightbox({
        title: (section && section.id ? "Edit" : "Create") + " page/section",
        message: "",
      });
      var container = lightbox.getContainer();
      container.find(".outer .outer").html(
        pageTemplate({
          obj: section,
          tpRo: tpRo,
          tpText: tpText,
      ...
      container.find("button[value=ok]").on("click", function () {
        var update = _.extend(
          helpers.unMap(container.find("form").serializeArray()),
          details
        );
        intify(update, "order");
        $.ajax({
      ...
      container.find("button[value=createChild]").on("click", function () {
        delete details.page;
        editPageFull(
          { title: "<add title>", order: "10", parent_id: section.id },
          details
        );
      });
    ...
    var tp = _.template(
      '\
			...
			<div class="config-menu hidden">\
				<button type="button" value="toggle-edit">' +
        (editMode ? "Stop edit mode" : "Edit mode") +
        '</button>\
