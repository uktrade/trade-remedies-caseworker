...
class UserView(UserBaseTemplateView):
    ...
    user_fields = json.dumps({'Case':{
            'id':0,
            'name':0,
            'reference':0,
            'workflow_state': {
                'MEASURE_EXPIRY':0,
                'LATEST_MEASURE_EXPIRY':0
            }
    ...
    def get(self, request, user_id=None, user_group=None, *args, **kwargs):
        ...
        cases = []
        if user_id:
            cases = client.get_user_cases(archived='all', request_for=user_id, all_cases=False, fields=self.user_fields)
        str_now = timezone.now().strftime(settings.API_DATETIME_FORMAT)
        for case in cases:
            expiry = get(case,'workflow_state/LATEST_MEASURE_EXPIRY/0') or get(case,'workflow_state/MEASURE_EXPIRY/0')
    ...
    def post(self, request, user_id=None, user_group=None, *args, **kwargs):
        ...
        user.update(pluck(request.POST, ['name', 'phone', 'timezone', 'roles', 'job_title_id', 'active', 'set_verified']))
        user['country'] = request.POST.get('country', user.get('country_code'))
        user['groups'] = request.POST.getlist('roles')  # translation needed as the create write key doesn't match the update
        if not user_id:
            user['country_code'] = request.POST.get('country')  # bodge as create doesn't match update
        ...
        if not errors:
            try:
                ...
                return self.get(request, user_id=user_id, user_group=user_group, errors=errors, user=user)
            else:
                return HttpResponse(json.dumps({'result': response}))
        else:
            return self.get(request, user_id=user_id, user_group=user_group, errors=errors, user=user, *args, **kwargs)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not errors:
            try:
                ...
                response['alert'] = 'You have changed your password and will be logged out. Please log in using your updated password.'
...
class ContactLookupView(LoginRequiredMixin, TemplateView, TradeRemediesAPIClientMixin):
    def get(self, request, *args, **kwargs):
        ...
        client = self.client(request.user)
        return HttpResponse(json.dumps({'result': client.lookup_contacts(search) if search else client.get_contact(contact_id)}), content_type='application/json')
