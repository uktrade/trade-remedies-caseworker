...
class CaseBaseView(LoginRequiredMixin, GroupRequiredMixin, PermissionRequiredMixin, TemplateView, TradeRemediesAPIClientMixin):
    permission_required = []
    ...
    def get_documents(self, submission=None, all_versions=None):
        result = self._client.get_submission_documents(self.case_id, submission.get('id'), all_versions=all_versions)
        ...
        if all_versions:
            ...
            if this_version > 1:
                ...
                for document in sub_docs:
                    document['parent'] = parent_doc_idx.get(get(document, 'type/name') + '|' + get(document, 'name'))
        else:
            sub_docs = all_documents
        submission_documents = deep_index_items_by(sub_docs, 'type/key')
        document_conf_index = deep_index_items_by(submission_documents.get('respondent', []), 'confidential')
        ...
        #    previous_submission = self._client.get_submission_documents(self.case_id, submission['previous_version']['id'])
...
class CaseMilestoneDatesView(CaseBaseView):
    ...
    def add_page_data(self):
        ...
        existing_keys = [
            cm['key'] for cm in case_milestones
        ]
        context = {
...
class CaseTeamView(CaseBaseView):
    ...
    def add_page_data(self):
        ...
        return {
            ...
            'inactive_user_count': sum(user['active'] == False for user in all_users),
            'singleton_groups': [
                SECURITY_GROUP_TRA_HEAD_OF_INVESTIGATION,
                SECURITY_GROUP_TRA_ADMINISTRATOR,
            ]
...
class SubmissionsView(CaseBaseView):
    ...
    def consolidate_submissions(self, case, participants, submissions_by_party, counts, selected_tab):
        roles = []
        ...
        for role in self._client.get_case_roles():
            ...
            sort_key = 'submissions/0/received_at' if selected_tab == CASE_ROLE_AWAITING_APPROVAL else 'name'
    ...
    def divide_submissions(self, submissions):
        ...
        for submission in submissions:
            if get(submission, 'status/sent'):
                ...
                if not get(submission, 'status/draft') or get(submission, 'type/key') == 'application':  # customer draft should not be seen by investigators
    ...
    def add_page_data(self):
        ...
        submission_fields = json.dumps({
            'Submission': {
                ...
                'version': 0,
                'status': {
                    'name': 0, 'draft': 0, 'default':0
                },
                'sent_at': 0,
                'received_at': 0,
                'created_at': 0,
                'due_at': 0, 'tra_editable': 1,
                'organisation': {'id': 0}
            }
        })

        all_submissions = self._client.get_submissions(self.case_id, show_global=True, fields=submission_fields)
        submissions_by_type = deep_index_items_by(all_submissions, 'type/name')

        # Get submissions that have just been created by customer or are still in draft after creation
        ...
        draft_first_version_submissions = deep_index_items_by(draft_submissions, 'version').get('1') or []
        # Exclude these drafts from our list
        non_draft_submissions = [sub for sub in all_submissions if sub not in draft_first_version_submissions]
        # draft applications are included to allow a heads up view to the caseworker before it's submitted
        ...
        if self.sub_page:
            ...
            self.template_name = f'cases/submissions_new.html'
        ...
        context = {
            ...
            'counts': counts,
            'all_roles': self.consolidate_submissions(self.case, participants=participants, submissions_by_party=submissions_by_party, counts=counts, selected_tab=tab),
        ...
        if not submissions_by_type.get('application') and submissions_by_type.get('ex officio application'):
...
class SubmissionView(CaseBaseView):
    ...
    def add_page_data_old(self):
        ...
        self.template_name = f"cases/submission.html"
        case_extras = self._client.get_case(self.case_id, fields=self.extra_case_fields)
        context = {
        ...
        if not submission or not submission.get('status') or submission.get('status', {}).get('default'):
            context['mode'] = 'form'
        else:
            context['mode'] = 'view'
    ...
    def add_page_data(self):
        ...
        if submission_id:
            ...
            if not (_default) or (_default and submission['type']['id'] == SUBMISSION_TYPE_APPLICATION):
            ...
            ret = {
                ...
                'json_data': json_data,
                'selected_submission_type': submission.get('type', {}).get('key') or 'questionnaire'
            }
        else:
            role = self.request.GET.get('for')
            sampled = self.request.GET.get('sampled') == 'sampled'
            full_role = self._client.get_case_role(role) if (role and role != 'public') else {'name': 'Public file'}
            ...
            draft_submissions = deep_index_items_by(all_submissions, 'status/default').get('true') or []
            # draft_submissions_this_role = deep_index_items_by(draft_submissions, 'organisation_case_role/key').get('' if role == 'public' else role)
            draft_submissions_this_role = deep_index_items_by(draft_submissions, 'organisation_id').get('')

            ret = {
                'submission': submission,
                'submission_type_id': self.kwargs.get('submission_type_id') or self.request.GET.get('submission_type_id'),
                'submission_statuses': case_enums['submission_statuses'],
                'statuses_by_type': case_enums['statuses_by_type'],
                'selected_submission_type': self.request.GET.get('submission_type') or 'questionnaire',
            ...
            if role == 'public':
                ...
                filtered = list(filter(lambda party: party if party.get('sampled') == sampled and not party.get('gov_body') else None, role_participants))
        ...
        template_name = submission['type']['key'] if submission else (role if role == 'public' else 'questionnaire')
        ret.update({
    ...
    def post(self, request, case_id, submission_id=None, organisation_id=None, *args, **kwargs):
        """
        ...
        if submission_id and btn_value == 'discard':
            ...
            return HttpResponse(json.dumps({'redirect_url': f'/case/{case_id}/submissions/'}), content_type='application/json')
        # check if the update is for name or notify contact
        if submission['name'] != name or not submission['contact'] or \
                submission.get('contact', {}).get('id') != contact_id:
        ...
        if submission.get('id'):
            ...
            if btn_value in ['send', 'publish', 'withdraw']:
                if btn_value in ['publish', 'withdraw']:
                    result = self._client.set_submission_state(case_id, submission_id, 'sent' if (btn_value == 'send' or submission_group_name == 'public') else '', {'publish': 'issue', 'withdraw': 'un-issue'}[btn_value])
            ...
            if btn_value == 'sufficient':
                ...
                if type_helpers:
                    return_data.update(type_helpers(
                        submission, self.request.user
                    ).on_approve() or {})
...
class SubmissionCreateView(SubmissionView):
    ...
    def post(self, request, case_id, *args, **kwargs):
        ...
        submission_data = {
            'submission_type': int(request.POST.get('submission_type_id', SUBMISSION_TYPE_QUESTIONNAIRE)),
            'case_id': str(case_id),
        ...
        if send_to:
            submission_data['deficiency_notice_params'] = to_json({
                'send_to': send_to,
                'case_role': request.POST.get('role_key')
            })
...
class SubmissionDocumentView(CaseBaseView):
    ...
    def post(self, request, case_id, submission_id, organisation_id=None, *args, **kwargs):
        ...
        if document_list_json:  # change state of existing documents
            document_list = json.loads(document_list_json)
            for doc_id, status in document_list.items():
                response = self._client.set_submission_document_status(
                    case_id=case_id,
                    submission_id=submission_id,
                    document_id=doc_id,
                    status=status
...
class SubmissionDeficiencyView(CaseBaseView):
    ...
    def get(self, request, case_id, submission_id, *args, **kwargs):
        ...
        organisation_name = submission.get('organisation', {}).get('name') or (contact.get('organisation') or {}).get('name')
        notification_template = self._client.get_notification_template('NOTIFY_SUBMISSION_DEFICIENCY')
    ...
    def post(self, request, case_id, submission_id, *args, **kwargs):
        ...
        submission = self._client.get_submission(case_id, submission_id)
        notify_keys = [
            'full_name', 'case_name', 'case_number', 'company_name',
            'deadline', 'submission_type', 'login_url'
        ]
        ...
        return HttpResponse(json.dumps({
            'error': f'No deficiency status for this submission type',
        }), content_type='application/json')
...
class SubmissionVerifyBaseView(CaseBaseView):
    ...
    def get_submission_id(self, case_id=None, organisation_id=None):
        ...
        if not submission_id:
            # If this is called from the party page - there is no submission id - so find from the org/case
...
class SubmissionVerifyViewTasks(SubmissionVerifyBaseView):
    ...
    def get(self, request, case_id, organisation_id, **kwargs):
        ...
        if not submission_id:
            return HttpResponse(json.dumps({
                'error': f'You cannot verify this organisation as they have not yet registered interest in this case.',
            }), content_type='application/json')
        submission = self._client.get_submission(self.case_id, submission_id, fields=self.submission_fields)
        json_data = submission.get('deficiency_notice_params') or {}
        organisation = submission.get('organisation')
        caserole = self._client.get_organisation_case_role(case_id=case_id, organisation_id=get(submission, 'organisation/id'))
...
class SubmisisonVerifyEditLoaView(SubmissionVerifyBaseView):
    ...
    def get(self, request, case_id, organisation_id):
        ...
        organisation = self._client.get_organisation(case_id=case_id, organisation_id=organisation_id)
        documents = self.get_documents(submission)
        caserole = self._client.get_organisation_case_role(case_id=self.case_id, organisation_id=organisation_id)
        org_contacts = self._client.get_organisation_contacts(organisation_id, case_id, exclude_indirect=True)

        return render(request, 'cases/verify/loa.html', {
    ...
    def post(self, request, case_id, organisation_id, *args, **kwargs):
        ...
        result = self._client.set_organisation_case_role_loa(case_id, organisation_id, pluck(request.POST, ['LOA_contact_id', 'name', 'email', 'address', 'org_name', 'phone']))
...
class SubmisisonVerifyOrganisation(SubmissionVerifyBaseView):
    ...
    def get(self, request, case_id, organisation_id):
        ...
        org_matches.sort(key=lambda m: 1 if m.get('id') == test_org_id else 0)  # put the actual match at the end
        ...
        return render(request, 'cases/verify/merge_org.html' if self.enable_merge else 'cases/verify/verify_org.html', {
            'case_id': self.case_id,
            'organisation': organisation,
            'match_list': matches,
    ...
    def post(self, request, case_id, organisation_id, *args, **kwargs):
        ...
        if verify == 'verified':
            self._client.verify_caserole(case_id=case_id, organisation_id=get(submission, 'organisation/id'))
...
class SubmissionVerifyAccept(SubmissionVerifyBaseView):
    ...
    def get(self, request, case_id, organisation_id):
        ...
        organisation = self._client.get_organisation(case_id=case_id, organisation_id=organisation_id)
        caserole = self._client.get_organisation_case_role(case_id=self.case_id, organisation_id=organisation_id)
        roles = self._client.get_case_roles(exclude=[CASE_ROLE_APPLICANT, CASE_ROLE_AWAITING_APPROVAL, CASE_ROLE_REJECTED, CASE_ROLE_PREPARING])

        return render(request, 'cases/verify/accept.html', {
            'submission': submission,
            'organisation': organisation,
            'roles': roles,
            'caserole': caserole,
    ...
    def post(self, request, case_id, organisation_id, *args, **kwargs):
        ...
        result = {}
        result = self._client.set_organisation_case_role(case_id, organisation_id, role_key, pluck(request.POST, ['approve']))
        return HttpResponse(json.dumps(result))
...
class SubmissionVerifyNotify(SubmissionVerifyBaseView):
    ...
    def get(self, request, case_id, organisation_id):
        caserole = self._client.get_organisation_case_role(case_id=self.case_id, organisation_id=organisation_id)
        role_name = get(caserole, 'role/name')
        action = 'reject' if get(caserole, 'role/key') == 'rejected' else 'accept'  # Todo: get this from the right place
        submission_id = self.get_submission_id(case_id=case_id, organisation_id=organisation_id)
        submission = self._client.get_submission(case_id, submission_id)
        case = self._client.get_case(case_id)
        contact = submission_contact(submission)
        organisation = self._client.get_organisation(case_id=case_id, organisation_id=organisation_id)
        notify_key = 'NOTIFY_INTERESTED_PARTY_REQUEST_PERMITTED' if action == 'accept' else 'NOTIFY_INTERESTED_PARTY_REQUEST_DENIED'
        try:
        ...
        return render(request, 'cases/verify/notify.html', {
...
class SubmissionNotifyView(CaseBaseView):
    ...
    def get(self, request, case_id, submission_id, *args, **kwargs):
        ...
        template_list = []
        if send_to:
            for case_role, participant_list in (self._client.get_case_participants(case_id) or {}).items():
                    ...
                    if participant.get('id') in send_to:
                        ...
                        if contact:
                            ...
                            template_list.append({
                                'values': local_values,
                                'preview': parse_notify_template(notification_template['body'], values)
                            })
        else:
            template_list[contact.get('email')] = parse_notify_template(notification_template['body'], values)
    ...
    def post(self, request, case_id, submission_id, *args, **kwargs):
        ...
        notify_keys = [
            'full_name', 'product', 'submission_request_name', 'description'
        ]
    ...
    def post_multiple(self, request, case_id, submission, context=None):
        ...
        json_data = from_json(submission.get('deficiency_notice_params'))
        send_to = json_data.get('send_to')
        # We need to know which is the last party in the list so we can modify the existing sub rather than clone it.
                ...
                if participant.get('id') in send_to:
                    ...
                    if contact: # don't try to send if there is no contact
                        ...
                        if party_counter:
                            ...
                            cloned_submission = self._client.update_submission(**data).get('submission')
        ...
        return HttpResponse(json.dumps({
            ...
            'redirect_url': f'/case/{case_id}/submission/{submission.get("id")}/' if len(send_to) < 2 else f'/case/{case_id}/submissions/',
            'error': None
        }), content_type='application/json')
...
class OrganisationDetailsView(LoginRequiredMixin, View, TradeRemediesAPIClientMixin):
    ...
    def get(self, request, case_id, organisation_id, *args, **kwargs):
        ...
        if template:
            deep_update(result,{'case_id': case_id, 'case': {'id': case_id}, 'organisation': {'id': organisation_id}})
...
class CaseOrganisationView(CaseBaseView):
    ...
    def add_page_data(self):
        ...
        roi_app_submission = next(filter(lambda x: get(x, 'type/key') in ['interest', 'application'], submissions), None)
...
class OrganisationMatchView(CaseBaseView):
    ...
    def add_page_data(self):
        organisation = self._client.get_organisation(organisation_id=self.organisation_id, case_id=self.case_id)
...
class FilesView(CaseBaseView):
    ...
    def add_page_data(self):
        ...
        collapse_identical = self.request.GET.get('collapse_identical', 'false') in ('true', '1', 'Y')
        tabs = {
            'tabList': [
...
class FileBrowseView(View, TradeRemediesAPIClientMixin):
    ...
    def get(self, request, case_id, *args, **kwargs):
        ...
        case_files = _client.get_case_documents(
            case_id=case_id,
            source='investigator')
...
class StateView(CaseBaseView):
    ...
    def post(self, request, case_id, state_key=None, *args, **kwargs):
        ...
        return HttpResponse(json.dumps({'workflow_state': state_map}), content_type='application/json')
...
class ActionView(CaseBaseView):
    ...
    def post(self, request, case_id, action_id=None, *args, **kwargs):
        values = {}
        node_keys = []
        ...
        for task in action.get('children', []):
            ...
            if response_type.lower() not in ('notesection', 'timer', 'label'):  # notes don't count as in-progress
        ...
        if error:
            ...
            return HttpResponse(json.dumps({'workflow_state': self.state_map}), content_type='application/json')
...
class NavSectionView(CaseBaseView):
    ...
    def post(self, request, case_id, *args, **kwargs):
        ...
        response = self._client.set_case_content(case_id, content_id=content_id, content=request.POST)
...
class NoteView(LoginRequiredMixin, View, TradeRemediesAPIClientMixin):
    ...
    def get(self, request, case_id, content_type=None, model_id=None, model_key=None, *args, **kwargs):
        notes = self.client(request.user).get_notes(case_id, content_type, model_id, model_key=model_key)
        ...
        if note_id is None:
            ...
            if conf_list:
                for document_id in conf_list:
                    result = client.update_note_document(case_id, note_id, document_id, 'confidential')
            nonconf_list = request.POST.getlist('set_non-confidential')
            if nonconf_list:
                for document_id in nonconf_list:
                    result = client.update_note_document(case_id, note_id, document_id, 'non-confidential')
...
class CaseFormView(LoginRequiredMixin, TemplateView, TradeRemediesAPIClientMixin):
    ...
    def post(self, request, case_id=None, *args, **kwargs):
        ...
        non_required_fields = [
            'submission_status_id', 'case_name', 'organisation_name',
            'organisation_id',
        ...
        for field in case_fields:
            post_data[field] = compact_list(request.POST.getlist(field)) if field in list_fields else request.POST.get(field)
...
class InviteContactView(CaseBaseView):
    ...
    def get_organisation_admin_user_contact(self, organisation_id):
        ...
        admin_user = [user for user in organisation.get('users', []) if user.get('security_group') == SECURITY_GROUP_ORGANISATION_OWNER]
        if admin_user:
    ...
    def add_page_data(self):
        ...
        if self.kwargs.get('contact_id'):
            ...
            form_url = f"/case/{self.case['id']}/invite/{self.kwargs['contact_id']}/as/{self.kwargs['case_role_id']}/"
            if organisation:
            ...
            form_url = f"/case/{self.case['id']}/invite/organisation/{self.kwargs['organisation_id']}/as/{self.kwargs['case_role_id']}/"
        if not organisation:
            organisation = contact['organisation']
        notification_template = self._client.get_notification_template('NOTIFY_INFORM_INTERESTED_PARTIES')

        deep_update(self.case, self._client.get_case(self.case_id, fields=json.dumps({
            'Case': {
        ...
        values = {
            ...
            'company_name': organisation['name'],
            'deadline': parse_api_datetime(get(self.case,'registration_deadline'), settings.FRIENDLY_DATE_FORMAT),
            'footer': self._client.get_system_parameters('NOTIFY_BLOCK_FOOTER')['value'],
    ...
    def post(self, request, contact_id=None, case_id=None, case_role_id=None, organisation_id=None, *args, **kwargs):
...
class CaseBundleView(CaseBaseView):
    ...
    def post(self, request, case_id, bundle_id=None, *args, **kwargs):
        name = request.POST.get('name')
...
class SubmissionInviteNotifyView(CaseBaseView):
    ...
    def get(self, request, case_id, submission_id, contact_id, *args, **kwargs):
        ...
        context = {
            'form_action': f"/case/{case_id}/submission/{submission_id}/invite/{contact_id}/notify/",
            'form_title': f"Invite 3rd party to {case['name']} on behalf of {inviting_organisation['name']}",
            'cancel_redirect_url': f"/case/{case_id}/submission/{submission_id}/",
            'editable_fields': {
                'full_name': {'title': 'Name'},
...
class UpdateParentView(CaseBaseView):
    ...
    def post(self, request, case_id,  *args, **kwargs):
        ...
        if get(case, 'parent/id') != parent_id:
            _client.set_case_data(case_id, {
                'parent_id': parent_id
            })
        if (get(case, f'workflow_state/{self.linked_case_confirm_key}') or [0])[0] != link_confirm:
            _client.set_case_workflow_state(case_id, values={f'{self.linked_case_confirm_key}': link_confirm})
        return HttpResponse(json.dumps({
            'parent_id': parent_id,
...
class NoticesView(LoginRequiredMixin, GroupRequiredMixin, TemplateView, TradeRemediesAPIClientMixin):
