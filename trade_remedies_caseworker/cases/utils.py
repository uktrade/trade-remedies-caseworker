...
def decorate_orgs(org_list, representing_org_id, exclude_case_id=None):
    ...
    for org in org_list:
        ...
        if usercases:
            org['cases_for_org'] = list([case for case in usercases if get(case, 'organisation/id') == representing_org_id])
        org['verified_usercases'] = list(verified_cases.values())
        org['verified_usercases'].sort(key=lambda uc: get(uc, 'caserole/validated_at', ''), reverse=True)
