...
contact_fields = [
    ...
    'contact_post_code', 'contact_country', 'primary_contact',
]

org_fields = [
    'organisation_name', 'companies_house_id', 'organisation_address',
    'organisation_post_code', 'organisation_type', 'vat_number', 'eori_number',
    'duns_number', 'organisation_website', 'organisation_country',
    'organisation_id', 'json_data',
]
...
class OrganisationsView(BaseOrganisationTemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        "value": tab,
            "tabList": [
                {"label": "Active organisations", "value": "active", "sr_text": "Organisations that have case participations" },
                {"label": "Fraudulent", "value": "fraudulent", "sr_text": "Organisations that have been marrked as fraudulent"},
                {"label": "No case participation", "value": "inactive", "sr_text": "Organisations tha have never participated in a case"}
            ]
        }
...
class OrganisationFormView(BaseOrganisationTemplateView):
    ...
    def get(self, request, case_id=None, organisation_id=None, organisation_type=None, *args, **kwargs):
        role = {}
    ...
    def post(self, request, case_id=None, organisation_id=None, organisation_type=None, *args, **kwargs):
        ...
        if organisation_id:
            ...
            organisation = get(self._client.update_case_organisation_by_type(case_id, organisation_type, org_request_fields), 'organisation')
            if not org_request_fields.get('organisation_id') and organisation:
                contact_request_fields = collect_request_fields(request, contact_fields, extra_context)
                contact_request_fields['organisation_id'] = organisation['id']
                if contact_request_fields.get('contact_name') and contact_request_fields.get('contact_email'):
    ...
    def post(self, request, case_id=None, organisation_id=None, contact_id=None, *args, **kwargs):
        ...
        try:
            if contact_id:
                ...
                response = self._client.create_and_add_contact(case_id, organisation_id, request_fields)
...
class ToggleOrganisationNonResponsiveView(BaseOrganisationTemplateView):
    ...
    def post(self, request, organisation_id, case_id, *args, **kwargs):
        ...
        org_name = (response.get('organisation') or {}).get('name')
        responsive_str = 'marked as non-cooperative' if response.get('non_responsive') else 'marked as cooperative'
...
class OrganisationCaseRoleView(BaseOrganisationTemplateView):
    ...
    def get(self, request, case_id, organisation_id, *args, **kwargs):
        ...
        notify_key = 'NOTIFY_COMPANY_ROLE_CHANGED' if action in ('approve', 'change') else 'NOTIFY_COMPANY_ROLE_DENIED'
        ...
        context = {
            ...
            'organisation_type': request.GET.get('organisation_type') or CASE_ROLE_AWAITING_APPROVAL,
        }
        return render(request, self.template_name, context)
...
class OrganisationRemoveView(BaseOrganisationTemplateView):
    ...
    def post(self, request, organisation_id, case_id, *args, **kwargs):
        ...
        """
        result = self._client.remove_organisation_from_case(case_id=case_id, organisation_id=organisation_id)
        party_name = result.get('name', 'unknown')
...
class OrganisationDuplicatesView(LoginRequiredMixin, View, TradeRemediesAPIClientMixin):
    ...
    def get(self, request, organisation_id, *args, **kwargs):
        org_matches = self.client(self.request.user).get_organisation_matches(organisation_id, with_details='none')
...
class OrganisationMatchView(BaseOrganisationTemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        matching_orgs = self._client.get_organisation_matches(name=organisation_name, companies_house_id=companies_house_id)
        return render(request, self.template_name, {
            'case': None,
            'organisation': None,
